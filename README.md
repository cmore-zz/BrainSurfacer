...
Org, or BBCode parsers without admitting arbitrary text files globally. An
override may instead auto-detect those formats from conservative evidence in
the first 128 KiB. Low-confidence and ambiguous files stay unindexed, and only
a confident match causes the complete file to be read and parsed. It
...
fingerprints after format mapping, detection, or parsing behavior changes.
Confidently skipped auto-detection candidates are fingerprinted too, so
unchanged plain text is not repeatedly probed; fingerprints created before
parser identifiers or suffixes were recorded are reparsed once.
